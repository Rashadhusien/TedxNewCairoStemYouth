...
async function checkPackageDiscount() {
  ...
  try {
    const packages = await sql.query(`
      SELECT id, name, price_per_ticket_piastres, discounted_price_per_ticket_piastres, is_active, is_promo_applicable
      FROM packages
    ...
    packages.forEach((pkg: any) => {
      ...
      const discountedPrice = pkg.discounted_price_per_ticket_piastres
        ? pkg.discounted_price_per_ticket_piastres / 100
        : null;

      console.log(`${pkg.name}:`);
      console.log(`  Regular price: ${regularPrice} EGP`);
      console.log(
        `  Discounted price: ${discountedPrice ? discountedPrice + " EGP" : "Not set"}`,
      );
      console.log(`  Active: ${pkg.is_active}`);
      console.log(
        `  Promo applicable: ${pkg.is_promo_applicable ? "YES" : "NO"}`,
      );
